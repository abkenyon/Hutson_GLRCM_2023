...
class CoordinateSystem(object):
    ...
    Each coordinate system must be able to convert data to a common coordinate system,
    which is chosen to be ECEF cartesian.
    data -> common system
    ...
    User code is responsible for taking data in its native coord system, 
        transforming it using to/fromECEF using the a coord system appropriate to the data,
        and then transforming that data to the final coordinate system using another coord
        system.
    ...
    The ECEF system has its origin at the center of the earth, with the +Z toward the north
    pole,
    ...
    def fromECEF(self, x, y, z):
        """
        Take ECEF x, y, z values and return x, y, z in the coordinate system defined by the
        object subclass
        """
        raise NotImplemented
    ...
    def toECEF(self, x, y, z):
        """
        Take x, y, z in the coordinate system defined by the object subclass and
        return ECEF x, y, z
        """
        raise NotImplemented
...
class GeographicSystem(CoordinateSystem):
    """
    Coordinate system defined on the surface of the earth using latitude, longitide, and
    altitude, referenced to WGS84 ellipse
    """
    
    WGS84lla = proj4.Proj(proj='latlong', ellps='WGS84', datum='WGS84')
        
    def toECEF(self, lon, lat, alt):
        projectedData = array(proj4.transform(GeographicSystem.WGS84lla,
                                              CoordinateSystem.WGS84xyz, lon, lat, alt))
    ...
    def fromECEF(self, x, y, z):
        projectedData = array(proj4.transform(CoordinateSystem.WGS84xyz,
                                              GeographicSystem.WGS84lla, x, y, z))
        if len(projectedData.shape) == 1:
            return projectedData[0], projectedData[1], projectedData[2]
        else:
            return projectedData[0,:], projectedData[1,:], projectedData[2,:]
...
class MapProjection(CoordinateSystem):
    ...
    def __init__(self, projection='eqc', ctrLat=None, ctrLon=None, ellipse='WGS84',
                 datum='WGS84', **kwargs):
    ...
    def toECEF(self, x, y, z):
        ...
        y += self.cy
        z += self.cz
        projectedData = array(proj4.transform(self.projection, CoordinateSystem.WGS84xyz,
                                              x, y, z ))
        if len(projectedData.shape) == 1:
    ...
    def fromECEF(self, x, y, z):
        projectedData = array(proj4.transform(CoordinateSystem.WGS84xyz, self.projection,
                                              x, y, z))
        if len(projectedData.shape) == 1:
            px, py, pz = projectedData[0], projectedData[1], projectedData[2]
        else:
...
class RadarCoordinateSystem(CoordinateSystem):
    ...
    def __init__(self, ctrLat, ctrLon, ctrAlt, datum='WGS84', ellps='WGS84',
                 effectiveRadiusMultiplier=4./3.):
        ...
        # Equatorial radius  - WGS-84 value = 6378137.0
        self.Requator, foo1, foo2 = proj4.transform(self.lla, self.xyz, 0, 0, 0)
        # Polar radius  - WGS-84 value = 6356752.314
        foo1, foo2, self.Rpolar = proj4.transform(self.lla, self.xyz, 0, 90, 0)
        self.flattening = (self.Requator-self.Rpolar)/self.Requator

        # First eccentricity squared - WGS-84 value = 0.00669437999013
        self.eccen = (2.0-self.flattening)*self.flattening
    ...
    def getGroundRangeHeight(self, r, elevationAngle):
        ...
        # Radar altitude is tacked on at the end, which isn't part of
        # their derivation. At 100 km, it's
        # worth < 10 m range error total for a radar at 500 m MSL.
        # For 250 m gate spacing (typical at S-band),
    ...
    def fromECEF(self, x, y, z):
        ...
        # have height, ground range, azimuth. need to get elev angle and slant range from
        # ground range and height
...
class TangentPlaneCartesianSystem:
    ...
    def __init__(self, ctrLat, ctrLon, ctrAlt):
        ...
        aboveCenterECEF = array(proj4.transform(WGS84lla, WGS84xyz, ctrLon, ctrLat,
                                                self.ctrAlt+1))
        ...
        # Point just to the north of the center on earth's surface,
        # projected onto the tangent plane
        # This calculation seems like it should only be done with latitude/north
        # since the local x
        #   direction curves away along a non-straight line when projected onto the plane
        northCenterECEF = array(proj4.transform(WGS84lla, WGS84xyz, self.ctrLon,
                                                self.ctrLat+0.01, self.ctrAlt))
        localy = dot(P, northCenterECEF[:, None])
        localy = -localy / norm(localy)  # negation gets x and y pointing in the right
        # direction
        ...
        # (X1, X2, X3) are the direction cosines of the X-direction of the b-system,
        # expressed in function of X, Y and Z of the a-system
    ...
    def toLocal(self, data):
        """Transforms 3xN array of data (position vectors) in the ECEF sytem to the local
        tangent plane cartesian system.
           Returns another 3xN array 
        """
        return array([dot(self.TransformToLocal, (v-self.centerECEF)[:, None]) for v in
